...
fun MaterialTextView.setResizableText(
    ...
    val suffixText =
        if (viewMore) "...Read More" else {
            ""
        }
    var charactersToTake =
        charactersAtLineEnd - suffixText.length / 2 // Good enough first guess
...
private fun MaterialTextView.addClickablePartTextResizable(
    ...
    if (clickableText != null) {
        ...
        val startIndexOffset =
            if (viewMore) 2 else 0 // Do not highlight the 3 dots and the space
        builder.setSpan(
